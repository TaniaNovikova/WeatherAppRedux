...
function Search() {
  ...
  // u — флаг Unicode, который говорит движку JavaScript, 
  // что регулярное выражение должно корректно работать с символами Unicode
  //  (например, буквами с диакритическими знаками: é, ü, ø).
  // 2. ^
  // Начало строки.
  // Указывает, что проверка начинается с самого первого символа строки.
  //  Если строка начнется с неподходящего символа, она сразу не пройдет проверку.
  // 3. [\p{L}\s\-]
  // Группа символов, которые разрешены. Все, что находится внутри квадратных скобок [],
  //  означает "один из этих символов".
  ...
  // Дефис.
  // Соответствует символу дефиса (-), который часто используется в названиях городов, 
  // таких как San-José или Йошкар-Ола.
  // 4. +
  ...
  // 5. $
  // Конец строки.
  // Указывает, что проверка должна завершаться в конце строки.
  // Если строка содержит лишние символы после последнего допустимого символа, она не пройдет проверку.
